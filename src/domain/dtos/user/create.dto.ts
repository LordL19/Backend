...
export class CreateUserDto {
	...
	static create(object: Record<string, any>): CreateUserDto {
		const name = DtoValidation.get(object.name, "Name")
			...
			.asString()
			.value();
		const last_name = DtoValidation.get(object.last_name, "Last_name")
			.required()
			.asString()
			.value();
		const email = DtoValidation.get(object.email, "Email")
			...
			.asEmail()
			.value();
		const type = DtoValidation.get(object.type, "Type")
			.required()
			.asString()
			.value();
		if (!(type in Type))
			throw ResponseError.badRequest({
				type: `Type ${type} is not valid for types ${Object.values(Type)}`,
			});
